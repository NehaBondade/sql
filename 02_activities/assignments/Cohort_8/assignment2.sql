...
SELECT 
	v.vendor_name,
	p.product_name,
	SUM(vi.unit_price * 5) AS potential_revenue
FROM 
	vendor v
...
DELETE FROM 
	product_units
WHERE 
	snapshot_timestamp < (SELECT MAX(snapshot_timestamp) FROM product_units WHERE product_name = 'Apple Pie')
		...
		SELECT 
			COALESCE((
				...
				FROM 
					vendor_inventory vi
				WHERE 
					vi.product_id = pu.product_id
				ORDER BY 
					vi.last_restock_date DESC
				LIMIT 1
			), 0)
	)
WHERE 
